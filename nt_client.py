...
class NetworkTablesClient:
    def __init__(self, server: str = "127.0.0.1", period: float = 0.02) -> None:
        ...
        self._pose2d_sub = self._instance.getStructTopic(POSE2D_PATH, Pose2d).subscribe(Pose2d(), options)
        self._mechanism_sub = self._instance.getStructArrayTopic(MECHANISM_PATH, Pose3d).subscribe([], options)
        self._fuel_sub = self._instance.getStructArrayTopic(FUEL_PATH, Pose3d).subscribe([], options)

    def get_pose2d(self) -> Pose2dData:
        pose = self._pose2d_sub.get()
        if isinstance(pose, Pose2d):
            return Pose2dData(float(pose.X()), float(pose.Y()), float(pose.rotation().radians()))
